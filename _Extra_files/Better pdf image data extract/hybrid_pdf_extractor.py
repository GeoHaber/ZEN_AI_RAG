...
class UniversalExtractor:
    ...
    def process(
        ...
        logger.info(f"Processing {document_type.value}: {file_path.name}")
        self.stats = ProcessingStats()
        
        # Route to appropriate handler
        if document_type == DocumentType.PDF:
            return self._process_pdf(file_path, parallel)
        ...
        try:
            with fitz.open(pdf_path) as doc:
                if doc.is_encrypted and not doc.authenticate(""):
                    logger.error(f"Cannot decrypt: {pdf_path.name}")
                    return [], self.stats
            ...
            if parallel and self.stats.total_pages > 1:
                page_results = self._extract_pdf_parallel(pdf_path)
            else:
                page_results = self._extract_pdf_sequential(pdf_path)
            
            # Sort by page number
            page_results.sort(key=lambda x: x['page_num'])
            
            # Chunk all text
            return self._finalize_chunks(page_results, pdf_path.name)
    ...
    def _extract_pdf_sequential(self, pdf_path: Path) -> List[Dict]:
    ...
    def _extract_pdf_parallel(self, pdf_path: Path) -> List[Dict]:
        """Parallel PDF page processing."""
        results = []
        
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._process_pdf_page_worker, pdf_path, i): i
    ...
    def _process_pdf_page_worker(self, pdf_path: Path, page_num: int) -> Optional[Dict]:
        ...
        for sentence in sentences:
            ...
            if current_length + sent_len > self.chunk_size and current_sentences:
                ...
                chunks.append(self._create_chunk(
                    chunk_text, source, page_num, chunk_index
                ))
                chunk_index += 1
    ...
    def _finalize_chunks(
        ...
        """Convert page results to chunks."""
        all_chunks = []
