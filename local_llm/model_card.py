    ...
    def get_recommendations(self, use_case: str = 'balanced') -> List[ModelCard]:
        ...
        with self._lock:
            if use_case == 'fast':
                ...
                return sorted(
                    self.get_cards_by_category(ModelCategory.LARGE),
                    key=lambda m: m.context,
                    reverse=True
                )

            elif use_case == 'coding':
                return sorted(
                    [m for m in self.models if m.capabilities.coding],
                    key=lambda m: m.estimated_speed,
                    reverse=True
                )

            elif use_case == 'reasoning':
                return sorted(
                    [m for m in self.models if m.capabilities.reasoning],
                    key=lambda m: m.context,
                    reverse=True
                )
