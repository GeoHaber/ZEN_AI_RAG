...
def handle_error(error: Exception, context: str, notify_user: bool = True) -> None:
    """
    ...
    if notify_user:
        try:
            ...
            user_message = error_messages.get(type(error), f"An error occurred: {str(error)}")

            ui.notify(f"{EMOJI['error']} {user_message}", color="negative", position="top", timeout=5000)
