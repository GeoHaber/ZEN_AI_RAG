...
async def handle_telegram_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if full_response:
            await context.bot.edit_message_text(
                chat_id=update.effective_chat.id, message_id=status_msg.message_id, text=full_response
            )
        else:
            await context.bot.edit_message_text(
                chat_id=update.effective_chat.id,
        ...
        await context.bot.edit_message_text(
            chat_id=update.effective_chat.id, message_id=status_msg.message_id, text=f"❌ Gateway Error: {str(e)}"
        )
