...
class ImagePreprocessor:
    ...
    def detect_preprocessing_needs(img: np.ndarray) -> Dict[str, bool]:
        ...
        needs = {"denoise": False, "contrast": False, "binarize": False, "deskew": False, "scale": False}
    ...
    def analyze_visuals(img: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            std = np.std(gray)
            if std < 30:
                stats["contrast_level"] = "low"
            elif std > 80:
                stats["contrast_level"] = "high"
...
class _UniversalExtractorBase:
    ...
    def _process_pdf(
        self, input_data: Union[Path, bytes], parallel: bool, filename: str
    ) -> Tuple[List[ExtractedChunk], ProcessingStats]:
        ...
        try:
            ...
            open_args = (
                ...
                if isinstance(input_data, (str, Path))
                else {"stream": input_data, "filetype": "pdf"}
            )
    ...
    def _process_image(
        self, input_data: Union[Path, bytes], doc_type: DocumentType, filename: str
    ) -> Tuple[List[ExtractedChunk], ProcessingStats]:
        ...
        try:
            ...
            chunks = self._create_semantic_chunks(cleaned, filename, page_num=1, visual_meta=visual_stats)
    ...
    def _process_video(
        ...
        try:
            ...
            chunks = self._create_semantic_chunks(
                narrative, filename, page_num=1, visual_meta={"video_stats": video_data}
            )
...
class UniversalExtractor(_UniversalExtractorBase):
    ...
    def _extract_pdf_parallel(self, input_data: Union[Path, bytes]) -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._process_pdf_page_worker, input_data, i): i for i in range(self.stats.total_pages)
            }
    ...
    def _process_pdf_page(self, page, page_num: int, doc) -> Optional[Dict]:
        ...
        try:
            ...
            return {"page_num": page_num + 1, "text": cleaned + image_markdown, "is_ocr": needs_ocr}
    ...
    def _ocr_image(self, img: Union[np.ndarray, Image.Image]) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(pil_img, config=self.tesseract_config, timeout=self.ocr_timeout)
            return text
    ...
    def _create_semantic_chunks(
        self, text: str, source: str, page_num: int, visual_meta: Optional[Dict] = None
    ) -> List[ExtractedChunk]:
...
if __name__ == "__main__":
    extractor = UniversalExtractor(chunk_size=1000, chunk_overlap=200, max_workers=4, preprocess_images=True)
    ...
    safe_print("=" * 70)
    chunks, stats = extractor.process("screenshot.png", document_type=DocumentType.SCREENSHOT)
    safe_print(stats.summary())
