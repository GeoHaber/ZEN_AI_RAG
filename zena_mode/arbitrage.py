...
class AgentPerformanceTracker:
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            ...
            conn.close()
            return {"total_queries": count or 0, "avg_consensus": avg_cons or 0.0, "avg_confidence": avg_conf or 0.0}
        except Exception as e:
            ...
            for i in range(len(responses)):
                for j in range(i + 1, len(responses)):
                    if similarities[i][j] < 0.2:  # Threshold for major contradiction
                        contradictions.append({"pair": (i + 1, j + 1), "similarity": float(similarities[i][j])})
...
class SwarmArbitrator(_SwarmArbitratorBase):
    ...
    async def _query_model_with_timeout(
        self, client: httpx.AsyncClient, endpoint: str, messages: List[Dict], timeout: float = 30.0
    ) -> Dict:
    ...
    def should_do_round_two(self, agreement: float, confidence_scores: List[float]) -> bool:
        ...
        if agreement > 0.8:
            return False
        if sum(confidence_scores) / max(1, len(confidence_scores)) > 0.85:
            return False
        if self._calculate_variance(confidence_scores) < 0.1:
            return False
    ...
    async def get_cot_response(
        self, text: str, system_prompt: str, verbose: bool = False, task_type: str = "general"
        ...
        expert_messages = [{"role": "system", "content": expert_system_prompt}, {"role": "user", "content": text}]
        ...
        async with httpx.AsyncClient() as client:
            ...
            if len(self.endpoints) == 1:
                ...
                critique_msg = expert_messages + [
                    ...
                    {
                        "role": "user",
                        "content": "Critique your previous answer for logic, accuracy, and completeness. Then provide a final corrected version.",
                    },
                ]
                ...
                tasks = [
                    self._query_model_with_timeout(client, ep, expert_messages, timeout=30.0) for ep in self.endpoints
                ]
            ...
            referee_endpoint = (
                self.endpoints[0] if self.endpoints else f"http://{config.host}:{config.llm_port}/v1/chat/completions"
            )
            ...
            if contradictions:
                safe_print(f"  ALERTS: {len(contradictions)} major contradictions detected.")
            ...
            if contradictions:
                ...
                for c in contradictions:
                    contradiction_info += (
                        f"- Expert {c['pair'][0]} vs Expert {c['pair'][1]} (Similarity: {c['similarity']:.2f})\n"
                    )
...
"""
            ...
            try:
                async with client.stream(
                    "POST", referee_endpoint, json=payload, timeout=httpx.Timeout(60.0, connect=5.0)
                ) as response:
            ...
            for r in valid_results:
                ...
                if "[SOURCE]:" in text or "Reference Context:" in text:
                    ...
                    if fact_check["score"] < self.reliability_penalty_threshold:
                        logger.warning(
                            f"[Arbitrator] Penalizing {r['model']} for low fact-check score: {fact_check['score']}"
                        )
