...
class VoiceManagerWithHealing(VoiceManager):
    ...
    def diagnose_microphone(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        return self.healer.auto_heal_with_recommendations() if verbose else self.healer.full_diagnostic(verbose=False)
    ...
    def record_audio_with_healing(
        self, duration: float = 5.0, device_id: Optional[int] = None, auto_fallback: bool = True, verbose: bool = False
    ) -> Optional[bytes]:
        """
        ...
        result = self.record_audio(duration=duration, device_id=device_id, auto_fallback=auto_fallback)
        ...
        if auto_fallback and self.healer and verbose:
            ...
            if best is not None and best != device_id:
                ...
                result2 = self.record_audio(duration=duration, device_id=best, auto_fallback=False)
...
def _record_audio_with_healing_part1(self, auto_fallback, duration, verbose):
    ...
    if auto_fallback:
        ...
        if best_device is not None and best_device != self.device:
            if verbose:
                ...
                if proc_count > 0:
                    logger.warning(f"  {proc_count} audio-using processes detected")

                device_name = result["diagnostic"]["best_device"]["device_name"]
            ...
            try:
                audio = self.record_audio(duration=duration, auto_fallback=False)
                if audio:
                    if verbose:
    ...
    def record_audio_with_healing(
        self, duration: float = 5.0, auto_fallback: bool = True, verbose: bool = False
    ) -> Optional[bytes]:
        """
        Record audio with automatic microphone healing.
        ...
        if not self.healer:
            ...
            return self.record_audio(duration=duration, auto_fallback=auto_fallback)
