...
class UITutorial:
    ...
    def __init__(self, client):
        ...
        self.steps = [
            {"title": "Welcome to ZenAI! 🚀", "message": "Let's take a 1-minute tour of the key features.", "element_id": None},
            {"title": "The Brain (Chat Hub) 🧠", "message": "Type your questions here. RAG searches your local docs first!", "element_id": UI_IDS.INPUT_CHAT},
            {"title": "Attach & Analyze 📎", "message": "Upload PDFs or Images. I'll OCR them instantly.", "element_id": UI_IDS.BTN_ATTACH},
            {"title": "Voice Intelligence 🎙️", "message": "Use the voice button for speech-to-text input.", "element_id": UI_IDS.BTN_VOICE},
            {"title": "Knowledge Management 📚", "message": "Manage RAG sources, switch models, or toggle Swarm mode in the drawer.", "element_id": "ui-drawer-btn"},
            {"title": "Settings ⚙️", "message": "Configure language and AI behavior here.", "element_id": UI_IDS.BTN_SETTINGS},
    ...
    async def _run_step(self):
        ...
        if self.title_label: self.title_label.text = step["title"]
        if self.message_label: self.message_label.text = step["message"]
