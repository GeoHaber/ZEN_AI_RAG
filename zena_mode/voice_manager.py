...
class VoiceManager:
    ...
    def __init__(
        self, model_dir: Optional[Path] = None, stt_model: str = "base.en", tts_voice: str = "en_US-lessac-medium"
    ):
        """
            ...
            if self._voice_service is None:
                try:
                    self._voice_service = VoiceService(
                        model_dir=self.model_dir, stt_model=self.stt_model, tts_voice=self.tts_voice
                    )
...
def _record_audio_part1(self):
    ...
    return RecordingResult(
        success=False, error=f"Recording failed on all {len(devices_to_try)} devices. Last error: {last_error}"
    )
    ...
    def record_audio(
        ...
        if not HAS_AUDIO_CAPTURE:
            return RecordingResult(
                success=False, error="sounddevice not installed - run: pip install sounddevice scipy"
            )
        ...
        for attempt, dev_id in enumerate(devices_to_try, 1):
            try:
                logger.info(
                    f"[VoiceManager] Recording {duration}s from device {dev_id} (attempt {attempt}/{len(devices_to_try)})"
                )
                ...
                result = RecordingResult(
                    success=True, audio_data=wav_buffer.getvalue(), duration=duration, sample_rate=sample_rate
                )
