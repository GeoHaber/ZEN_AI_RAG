    ...
    def _load_metadata(self):
        ...
        try:
            points, _ = self.qdrant.scroll(
                collection_name=self.collection_name, limit=10000, with_payload=True, with_vectors=False
            )
...
class LocalRAG(_LocalRAGBase):
    ...
    def _find_near_duplicate(self, embedding: "np.ndarray", threshold: float) -> bool:
        ...
        try:
            results = self.qdrant.query_points(
                collection_name=self.collection_name, query=embedding.tolist(), limit=1, score_threshold=threshold
            ).points
    ...
    def chunk_documents(
        self, documents: List[Dict], chunk_size: int = 500, overlap: int = 50, filter_junk: bool = True
    ) -> List[Dict]:
        ...
        for doc in documents:
            ...
            doc_chunks = self.chunker.chunk_document(
                content, metadata=meta, strategy=config.rag.chunk_strategy, filter_junk=filter_junk, model=self.model
            )
    ...
    def search(self, query: str, k: int = 5, rerank: bool = True) -> List[Dict]:
        ...
        hits = self.qdrant.query_points(collection_name=self.collection_name, query=query_vec, limit=limit).points
    ...
    def hybrid_search(self, query: str, k: int = 5, alpha: float = 0.5, rerank: bool = True) -> List[Dict]:
        ...
        if self.qdrant:
            ...
            hits = self.qdrant.query_points(
                collection_name=self.collection_name, query=query_vec, limit=k_search
            ).points
        else:
        ...
        if self.bm25:
            ...
            pos_indices = sorted([(i, s) for i, s in enumerate(scores) if s > 0], key=lambda x: x[1], reverse=True)[
                :k_search
            ]
...
def generate_rag_response(
    query: str, rag: LocalRAG, llm_backend, use_hybrid: bool = True, k: int = 5, alpha: float = 0.6
