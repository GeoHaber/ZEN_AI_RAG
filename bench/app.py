...
from zen_core_libs.rag.reranker_advanced import AdvancedReranker
...
@app.route('/shared-ui/<path:filename>')
def shared_ui_route(filename):
    from flask import send_from_directory
    shared_dir = Path(__file__).parent.parent / "zen_core_libs" / "zen_core_libs" / "ui"
...
PIPELINE_PRESETS = {
    ...
    "enterprise_stack": {
        ...
        "desc": "HyDE Query Expansion + Advanced Reranking + CRAG + Hallucination check.",
        "color": "#e11d48",
        "rerank": True, "dedup": True, "query_routing": True,
        "hallucination_check": True, "corrective_rag": True,
        "use_hyde": True, "use_advanced_reranker": True,
    },
}
...
def _pipeline_retrieve(pid: str, query: str, k: int = 5):
    ...
    fetch_k = k * 3 if (cfg.get("rerank") or cfg.get("dedup")) else k

    routing = None
    if cfg.get("query_routing"):
    ...
    if cfg.get("use_hyde"):
        ...
        def _llm_gen(prompt):
            try:
                ...
                resp = http_requests.post(api_url, 
                    json={"model": llm["model"], "messages": [{"role": "user", "content": prompt}], "max_tokens": 200},
                    headers={"Authorization": f"Bearer {llm['api_key']}"}, timeout=10)
                if resp.ok:
        ...
        try:
            hyde = HyDERetriever(
                llm_fn=_llm_gen, 
                embed_fn=lambda t: t, 
                search_fn=lambda t, kk: [{"text": r.chunk.text, "score": r.score, "chunk": r.chunk} for r in INDEX.search(t, k=kk)]
            )
            st_results = [{"text": r.chunk.text, "score": r.score, "chunk": r.chunk} for r in results]
            fused = hyde.retrieve_with_fusion(query, st_results, top_k=fetch_k)
            
            SearchResult = type(results[0]) if results else None
            if SearchResult:
                results = [SearchResult(chunk=r_dict["chunk"], score=r_dict["fusion_score"]) for r_dict in fused]
                timing["hyde_fused"] = True
        except Exception as exc:
            logger.warning("HyDE failed: %s", exc)

    if cfg.get("corrective_rag") and results:
        chunk_dicts = [
    ...
    if cfg.get("rerank") and results:
        ...
        if cfg.get("use_advanced_reranker") and ADVANCED_RERANKER:
            chunk_dicts = [{"text": r.chunk.text, "url": r.chunk.source_url, "title": r.chunk.page_title} for r in results]
            ranked_chunks = ADVANCED_RERANKER.rerank(query, chunk_dicts, top_k=k)
            text_to_result = {r.chunk.text: r for r in results}
            results = [text_to_result[c["text"]] for c in ranked_chunks if c["text"] in text_to_result]
        else:
...
def grade_site():
    ...
    if "gemini" not in llm["model"].lower():
        return jsonify({"error": "Website Grader requires a Gemini model to be active in Settings."}), 400
        
    try:
        from zen_core_libs.acquire.crawler import WebCrawler
        crawler = WebCrawler(timeout=10, extract_links=False)
        res = crawler.crawl_sync(url)
        content = res.content or ""
        if len(content) < 50:
            return jsonify({"error": "Not enough textual content to grade on the homepage."}), 400
            
        prompt = (
            f"You are an expert Website Designer and SEO Analyst. Evaluate the following homepage infrastructure and content.\n"
            f"1. Very first line, write exactly: GRADE: [A/B/C/D/F]\n"
            f"2. Provide exactly 3 bullet points with critique on Information Architecture & Health.\n"
            f"3. Provide exactly 3 bullet points with critique on UX & Content Design Improvements.\n\n"
            f"Website URL: {url}\n\nContent:\n{content[:6000]}"
        )
        
        from zen_core_libs.llm import GeminiAdapter
        adapter = GeminiAdapter(api_key=llm["api_key"], model_name=llm["model"])
        report = adapter.generate_text(prompt, system_prompt="You are an expert, tough, but fair Website Webmaster.", max_tokens=1024)
        
        return jsonify({"report": report})
    except Exception as e:
        logger.exception("Error grading site")
    ...
    def generate():
        ...
        try:
            if "gemini" in llm["model"].lower():
                from zen_core_libs.llm import GeminiChatbot
                bot = GeminiChatbot(api_key=llm["api_key"], model_name=llm["model"])
                yield f"data: {json.dumps({'sources': rag_sources, 'rag_timing': rag_timing})}\n\n"
                sys_content = system_msg["content"]
                for chunk in bot.chat_stream(system_prompt=sys_content, messages=trimmed_messages):
    ...
    def _worker(pid):
        ...
        try:
            ...
            if "gemini" in llm["model"].lower():
                from zen_core_libs.llm import GeminiChatbot
                bot = GeminiChatbot(api_key=llm["api_key"], model_name=llm["model"])
                sys_prompt = pd["system_msg"]["content"]
                for chunk in bot.chat_stream(system_prompt=sys_prompt, messages=trimmed):
                    full_text.append(chunk)
                    q.put({"pipeline": pid, "content": chunk})
            else:
                resp = http_requests.post(
