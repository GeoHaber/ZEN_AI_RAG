...
def _make_progress() -> dict:
    ...
    return {
        "bar": ft.ProgressBar(width=500, color=TH.accent, bgcolor=TH.card, value=0, visible=False),
...
def _progress_column(progress: dict) -> ft.Column:
    ...
    return ft.Column(
        [
            ft.Container(content=progress["bar"], width=500, alignment=ft.Alignment(0, 0)),
            progress["label"],
            progress["detail"],
        ],
...
async def _handle_folder_scan(page, state, progress, folder_field, limit_field, on_done):
    ...
    try:
        ...
        loop = asyncio.get_running_loop()
        text, images, sources = await loop.run_in_executor(
            None, lambda: extractor.scan_directory(folder, max_files=max_files, progress_cb=pcb)
        )
        state["rag_content"] = text
        state["rag_sources"] = sources
        state["rag_images"] = images
...
def _build_web_tab(page, state, progress, on_done):
    """Web URL scan card."""
    url_f = themed_text_field("Website URL", "https://example.com", ft.Icons.LANGUAGE, expand=True)
    ...
    return glass_card(
        ft.Column(
            [
                ...
                ft.Text("Scan Website", weight=ft.FontWeight.BOLD, size=16, color=TH.text),
                ft.Text("Enter a URL to crawl and index web pages", size=11, color=TH.muted),
                spacer(),
...
def _build_folder_tab(page, state, progress, on_done):
    ...
    dir_f = themed_text_field("Directory Path", "C:\\Documents", ft.Icons.FOLDER_OPEN, expand=True)
    ...
    return glass_card(
        ft.Column(
            [
                ...
                ft.Text("Scan Local Files", weight=ft.FontWeight.BOLD, size=16, color=TH.text),
                ft.Text("Browse a folder to extract and index documents", size=11, color=TH.muted),
                spacer(),
                ft.Row(
                    [
                        dir_f,
                        mf_f,
                        ft.IconButton(
                            icon=ft.Icons.FOLDER_OPEN, icon_color=TH.accent2, on_click=pick_folder, tooltip="Browse"
                        ),
                    ],
                    spacing=8,
                ),
                spacer(),
                accent_button("▶ Start Folder Scan", on_click=on_scan, color=TH.accent2),
            ],
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            spacing=8,
        ),
        ...
        [
            ...
            glass_card(
                ft.Column(
                    [
                        ft.Container(
                            content=ft.Icon(ft.Icons.MAIL_OUTLINE, color=email_color, size=30),
                        ...
                        ft.Text("IMAP Email Scan", weight=ft.FontWeight.BOLD, size=16, color=TH.text),
                        ft.Text("Connect to your inbox via IMAP", size=11, color=TH.muted),
                        spacer(6),
                        ft.Row([srv_f], spacing=8),
                        ft.Row([addr_f, pw_f], spacing=8),
                        ft.Row([fld_f, me_f, db_f], spacing=8),
                        spacer(),
                        accent_button("▶ Start Email Scan", on_click=on_imap_scan, color=email_color),
                    ],
                    horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                    spacing=8,
                ),
            ...
            glass_card(
                ft.Column(
                    [
                        ft.Container(
                            content=ft.Icon(ft.Icons.ATTACH_EMAIL_OUTLINED, color=TH.accent2, size=30),
                            bgcolor=ft.Colors.with_opacity(0.08, TH.accent2),
                            border_radius=12,
                            width=56,
                            height=56,
                        ...
                        ft.Text("Local Email Files", weight=ft.FontWeight.BOLD, size=16, color=TH.text),
                        ft.Text("Scan .eml or .mbox files from disk", size=11, color=TH.muted),
                        spacer(6),
                        ft.Row([path_f, local_limit_f], spacing=8),
                        spacer(),
                        accent_button("▶ Scan Local Emails", on_click=on_local_scan, color=TH.accent2),
                    ],
                    horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                    spacing=8,
                ),
...
def _build_email_tab(page, state, progress, on_done):
    ...
    # IMAP fields
    srv_f = themed_text_field("IMAP Server", "imap.gmail.com", ft.Icons.DNS_OUTLINED, expand=True)
    addr_f = themed_text_field("Email Address", "user@example.com", ft.Icons.EMAIL_OUTLINED, expand=True)
    pw_f = ft.TextField(
        label="Password",
    ...
    fld_f = ft.TextField(label="Folder", value="INBOX", width=140, border_color=TH.border, color=TH.text)
    me_f = num_field("Max Emails", "100")
    db_f = num_field("Days Back", "30")

    # Local email fields
    path_f = themed_text_field("Email File Path", "C:\\emails", ft.Icons.FOLDER_SPECIAL_OUTLINED, expand=True)
...
def build_data_inspector_panel(page: ft.Page, state: dict) -> ft.Column:
        ...
        source_tiles.append(
            ft.ExpansionTile(
                ...
                controls=[
                    ft.Container(
                        content=ft.Text(
                            content[max(0, i * 500) : (i + 1) * 500][:500] if content else "No content preview",
                            font_family=MONO_FONT,
                            size=11,
                            selectable=True,
                            color=TH.dim,
    ...
    return ft.Column(
        [
            ...
            ft.ListView(controls=source_tiles, expand=True, spacing=4, auto_scroll=False),
        ],
        spacing=10,
        expand=True,
    )
...
def build_db_contents_panel(page: ft.Page, state: dict) -> ft.Column:
    ...
    async def _load():
        ...
        try:
            ...
            for doc in docs:
                ...
                doc_list.controls.append(
                    ft.Container(
                        content=ft.Row(
                            [
                                ft.Text(f"{icon} {name}", size=12, color=TH.text, expand=True),
                                ft.Text(
                                ...
                                ft.Text(ts[:16].replace("T", " ") if ts else "", size=11, color=TH.muted, width=130),
                            ]
                        ),
                        bgcolor=TH.card,
            ...
            if not docs:
                doc_list.controls.append(ft.Text("No documents indexed yet.", color=TH.muted, size=12))
            page.update()
        except Exception as exc:
...
def build_cleanup_panel(page: ft.Page, state: dict) -> ft.Column:
        ...
        try:
            ...
            for doc in docs:
                ...
                if src in seen:
                    ...
                    seen[src] = doc
            if conflicts:
                status_text.value = f"⚠️ Found {len(conflicts)} potential duplicates"
                    ...
                    results_list.controls.append(
                        ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text(
                                        f"🔄 Duplicate: {src}", size=12, color=TH.warning_c, weight=ft.FontWeight.BOLD
                                    ),
                                    ft.Text(f"First indexed: {orig.get('uploaded_at', '?')}", size=11, color=TH.muted),
                                ]
                            ),
                            bgcolor=TH.card,
                            border=ft.Border.all(1, TH.border),
    ...
    return ft.Column(
        [
            ...
            ft.Text("Find and resolve conflicts in your knowledge base.", size=12, color=TH.muted),
            spacer(),
            ft.Row(
                [
                    accent_button("🔍 Run Cleanup Scan", on_click=on_run_cleanup, color=TH.accent2, width=220),
                    status_text,
                ],
                spacing=12,
            ),
    ...
    async def _load():
        ...
        try:
            ...
            if hasattr(rag, "semantic_cache") and hasattr(rag.semantic_cache, "get_stats"):
                ...
                info_text.value = f"Cache hits: {hits} · Misses: {misses} · Hit rate: {rate:.1f}%"
            else:
...
def build_dedup_panel(page: ft.Page, state: dict) -> ft.Column:
        ...
        try:
            ...
            if dupes:
                ...
                for name, _doc in dupes:
                    results_list.controls.append(
                        ft.Container(
                            content=ft.Text(f"🗑️ {name}", size=12, color=TH.warning_c),
                            bgcolor=TH.card,
                            border=ft.Border.all(1, TH.border),
                            border_radius=8,
                            padding=ft.Padding.symmetric(vertical=6, horizontal=10),
    ...
    return ft.Column(
        [
            ...
            ft.Row(
                [
                    accent_button("🔍 Check for Duplicates", on_click=on_run_dedup, color=TH.accent2, width=240),
                    status_text,
                ],
                spacing=12,
            ),
