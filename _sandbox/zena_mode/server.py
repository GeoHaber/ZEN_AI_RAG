...
class LogRelay(threading.Thread):
    ...
    """
    def __init__(self, process, prefix="[Engine]", chunk_size=4096):
    ...
    def run(self):
        ...
        try:
            while not self._stop_event.is_set():
                # Read chunk (blocking call in this thread, but won't block main app)
                # Note: process must be opened with text=True for this to work as expected
                chunk = self.process.stdout.read(self.chunk_size)
                
                if not chunk:
                    # Empty read usually means EOF or Process died
            ...
            # Flush remaining debris
            if buffer.strip():
                 logger.info(f"{self.prefix} {buffer.strip()}")

        except ValueError:
            # Handle potential closed file on shutdown
            pass
        except Exception as e:
            logger.error(f"{self.prefix} Log Relay Error: {e}")
        finally:
            # Ensure stdout is closed to release resources
            try:
...
def restart_with_model(name):
    ...
    # Use CREATE_NEW_CONSOLE to detach fully or use our new relay if we wanted to capture it.
    # For restarts, a new console is often safer to avoid parent-child entanglement.
...
def start_server() -> NoReturn:
    ...
    if not SERVER_EXE.exists():
        ...
        try:
            ...
            # Drain dummy server output too
            LogRelay(SERVER_PROCESS, prefix="[Dummy]").start()
    ...
    try:
        ...
        if SERVER_PROCESS and SERVER_PROCESS.poll() is None:
            ...
            try:
                ...
                # --- START DEADLOCK FIX ---
                # Start LogRelay to drain the pipe
                LogRelay(SERVER_PROCESS).start()
                # --- END DEADLOCK FIX ---
        ...
        for _ in range(30): # 30 seconds timeout
            ...
            if SERVER_PROCESS.poll() is not None:
                # Script crashed immediately (LogRelay handles logging output)
                safe_print(f"❌ LLM-Server CRASHED during startup. Check terminal or logs.")
                sys.exit(1)
            time.sleep(1)
