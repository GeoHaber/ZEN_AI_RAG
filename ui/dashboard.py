...
def build_performance_dashboard(ui_state):
    ...
    with (
        ui.row().classes(
            "w-full px-4 py-1 bg-gray-50/50 dark:bg-slate-800/50 backdrop-blur-sm border-b border-gray-100 dark:border-slate-800 gap-4 justify-center items-center"
        ),
        ...
        with ui.row().classes("items-center gap-1"):
            ...
            ui_state.rag_latency_label = ui.label("0ms").classes(
                "text-[10px] font-mono text-green-600 dark:text-green-400"
            )
