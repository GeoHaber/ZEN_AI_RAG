...
def render_header(ZENA_MODE, locale, app_state, drawer, dialogs):
    ...
    with ui.row().classes("w-full items-center justify-between mb-4"):
        ...
        # App Status (Drawer)
        with ui.row().classes(
            "items-center gap-1.5 px-2 py-0.5 rounded-full bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30"
        ):
            app_state["drawer_status_dot"] = ui.label("●").classes("text-green-500 animate-pulse text-xs")
            app_state["drawer_status_label"] = ui.label("ONLINE").classes(
                "text-[9px] font-black tracking-widest text-green-500"
            )

        # Settings Gear Button
        if "settings" in dialogs:
            ui.button(icon=Icons.SETTINGS, on_click=lambda: dialogs["settings"].open()).props(
                f"flat round dense id={UI_IDS.BTN_SETTINGS}"
            ).classes(Styles.TEXT_MUTED + " hover:text-blue-500 transition-colors")

        # Close button (mobile)
        ui.button(icon=Icons.CLOSE, on_click=lambda: drawer.hide()).props(
            f"flat round dense id={UI_IDS.BTN_CLOSE_DIALOG}"
        ).classes("md:hidden " + Styles.TEXT_MUTED)
...
def render_new_chat(locale, actions):
    ...
    # New Chat
    ui.button(
        locale.BTN_NEW_CHAT if hasattr(locale, "BTN_NEW_CHAT") else "💬 New Chat",
        icon="add_comment",
        on_click=actions.start_new_chat,
    ).classes("w-full mb-2 " + Styles.BTN_PRIMARY).props(f"unelevated id={UI_IDS.BTN_NEW_CHAT}")

    # Tour
    ui.button("🚀 Take Tour", icon="school", on_click=actions.start_tour).classes(
        "w-full mb-4 " + Styles.BTN_SECONDARY
    ).props(f"outline id=ui-tour-btn")
...
def render_appearance(locale, actions):
    ...
    with ui.expansion(locale.SETTINGS_CAT_APPEARANCE, icon="palette").classes("w-full mb-4").props("default-opened"):
        with ui.column().classes("w-full gap-2 py-2"):
            ui.switch(
                locale.SETTINGS_DARK_MODE, value=get_settings().appearance.dark_mode, on_change=actions.on_theme_change
            ).classes("text-sm").props(f"id={UI_IDS.SW_DARK_MODE}")
...
def render_rag(locale, app_state, actions):
    ...
    with ui.expansion(locale.SETTINGS_CAT_RAG, icon="library_books").classes("w-full mb-4").props("default-opened"):
        with ui.column().classes("w-full gap-2 py-2"):
            ...
            ui.switch(locale.SETTINGS_RAG_ENABLED, value=rag_enabled_val, on_change=actions.on_rag_toggle).classes(
                "text-sm"
            )
            ...
            scan_btn = (
                ...
                .classes("w-full " + Styles.TEXT_ACCENT)
            )
...
def render_batch(locale, backend, actions, UI_IDS):
    ...
    with ui.expansion(locale.BATCH_MENU, icon="batch_prediction").classes("w-full mb-4"):
        with ui.column().classes("w-full gap-2 py-2"):
            ...
            # File Input
            files_input = (
                ui.input(placeholder=locale.BATCH_FILES_PLACEHOLDER)
                .classes("w-full h-10 text-xs")
                .props(f"outlined dense id={UI_IDS.INPUT_BATCH_FILES}")
            )
            ...
            with progress_container:
                ...
                progress_bar = ui.linear_progress(value=0, show_value=False).classes(
                    "w-full " + Styles.BATCH_PROGRESS_BAR
                )

            # Enqueue Button
            # We defer execution to actions.start_batch but need to pass UI refs
            batch_btn = (
                ui.button(locale.BATCH_ENQUEUE, icon="play_circle")
                .props(f"flat dense id={UI_IDS.BTN_BATCH_START}")
                .classes("w-full " + Styles.TEXT_ACCENT)
            )
            batch_btn.on_click(
                lambda: actions.start_batch(files_input, progress_container, progress_label, progress_bar, batch_btn)
            )
...
def render_models(locale, actions, app_state, dialogs):
    ...
    with ui.expansion(locale.NAV_MODEL_MANAGER, icon="smart_toy").classes("w-full mb-2").props("default-opened"):
        with ui.column().classes("w-full gap-3 py-2"):
            ...
            selected_info = (
                actions.get_model_info(selected_model, MODEL_INFO)
                if selected_model
                else {
                    "name": "None",
                    "desc": "No model loaded",
                    "size": "-",
                    "icon": "❌",
                    "good_for": [],
                    "speed": "-",
            ...
            if current_models:
                ...
                with ui.scroll_area().classes("w-full max-h-48"):
                    for mf in current_models:
                        if mf == selected_model:
                            continue
                        info = actions.get_model_info(mf, MODEL_INFO)
                        with (
                            ui.card()
                            .classes(
                                "w-full p-2 mb-1 hover:shadow-md hover:bg-gray-100 dark:hover:bg-slate-700 transition-all cursor-pointer"
                            )
            ...
            try:
                ...
                ui.button("Clear FIFO Memory", icon="delete", on_click=fifo.clear).classes(
                    "w-full text-xs text-red-600"
                )
            except Exception:
                pass
            ...
            try:
                ...
                ui.button(
                    ...
                    icon="keyboard",
                    on_click=lambda: ui.notify(f"User input: {input_nonblocking('Enter value:', timeout=5.0)}"),
                ).classes("w-full text-xs text-purple-600")
            except Exception:
                pass
            # Download Section
            with ui.expansion("📥 " + locale.MODEL_DOWNLOAD_NEW, icon=Icons.DOWNLOAD).classes("w-full"):
                with ui.column().classes("w-full gap-2 py-2"):
                    ...
                    for m in download_models:
                        with ui.row().classes(
                            "w-full items-center justify-between p-1 hover:bg-gray-50 dark:hover:bg-slate-800 rounded"
                        ):
                            with ui.column().classes("gap-0"):
                                ui.label(m["name"]).classes("text-xs font-bold")
                                ui.label(m["size"]).classes("text-[10px] text-gray-400")
                            ui.button(icon=Icons.DOWNLOAD, on_click=lambda mod=m: actions.download_model(mod)).props(
                    ...
                    if "model" in dialogs:
                        ui.button(
                            "🔗 " + locale.MODEL_CUSTOM_DOWNLOAD,
                            icon=Icons.CLOUD_DOWNLOAD,
                            on_click=lambda: dialogs["model"].open(),
                        ).props("flat dense").classes("w-full " + Styles.TEXT_ACCENT)
    ...
    with ui.expansion("🧠 Smart Mode", icon="psychology").classes("w-full mb-2"):
        with ui.column().classes("w-full gap-3 py-2"):
            ...
            with ui.row().classes("items-center gap-2 px-2 py-1 rounded bg-green-50 dark:bg-green-900/20"):
                ...
                app_state["swarm_status"] = ui.label("Council Ready").classes(
...
def render_labs(actions):
    ...
    with ui.expansion("🔬 Labs", icon="science").classes("w-full mb-2"):
        with ui.column().classes("w-full gap-2 py-2"):
            ui.button("Voice Lab", icon=Icons.RECORD, on_click=actions.open_voice_lab).props(
                "flat dense align=left"
            ).classes("w-full " + Styles.TEXT_PRIMARY)

    with ui.expansion("⚖️ Intelligence Judge", icon="balance").classes("w-full mb-2"):
        with ui.column().classes("w-full gap-2 py-2 p-2"):
            ui.button("Run Evaluation", icon="analytics", on_click=actions.open_judge).props(
                f"flat dense align=left id={UI_IDS.BTN_JUDGE}"
            ).classes("w-full " + Styles.TEXT_PRIMARY)
    ...
    with ui.expansion(locale.NAV_SYSTEM, icon="build").classes("w-full mb-2"):
        with ui.column().classes("w-full gap-2 py-2"):
            ...
            with ui.row().classes("w-full gap-2 flex-wrap"):
                ui.button("Version", icon="update", on_click=actions.check_llama_version).props("flat dense").classes(
                    "flex-1"
                )
                ui.button("Benchmark", icon="speed", on_click=lambda: actions.run_benchmark(status_label)).props(
                    "flat dense"
                ).classes("flex-1")
                ui.button("Diagnose", icon="healing", on_click=lambda: actions.run_diagnostics(status_label)).props(
                    "flat dense"
                ).classes("flex-1")
