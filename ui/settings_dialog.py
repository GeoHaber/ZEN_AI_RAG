...
def create_settings_dialog(
    ...
    on_dark_mode_change: Optional[Callable[[bool], None]] = None,
):
    """
    ...
    with ui.dialog() as dialog, ui.card().classes("w-full max-w-3xl max-h-[90vh]"):
        ...
        # Scrollable Content
        with ui.scroll_area().classes("w-full flex-1 p-4"), ui.column().classes("w-full gap-6"):
            # ==================== LANGUAGE ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_LANGUAGE, icon="language").classes("w-full " + Styles.CARD_BASE),
                ...
                language_options = {
                    code: f"{info['native']} ({info['name']})" for code, info in available_locales.items()
                }
            ...
            # ==================== APPEARANCE ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_APPEARANCE, icon="palette").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # Dark Mode
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.appearance.dark_mode, on_change=_handle_dark_mode_change).props(
                        f"id={UI_IDS.SET_DARK_MODE}"
                    )
                ...
                with ui.row().classes("w-full gap-8"):
                    ui.checkbox(locale.SETTINGS_SHOW_AVATARS, value=config.appearance.show_avatars).bind_value(
                        config.appearance, "show_avatars"
                    )
                    ui.checkbox(locale.SETTINGS_ANIMATE_MESSAGES, value=config.appearance.animate_messages).bind_value(
                        config.appearance, "animate_messages"
                    )

            # ==================== AI MODEL ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_AI_MODEL, icon="smart_toy").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # Default Model
                ...
                ui.input(value=config.ai_model.default_model, placeholder="e.g., qwen2.5-7b-q4_k_m.gguf").bind_value(
                ...
                # Temperature
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.ai_model.temperature, min=0, max=2, step=0.1, format="%.1f").bind_value(
                        config.ai_model, "temperature"
                    ).classes("w-24")
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.ai_model.max_tokens, min=128, max=32768, step=256).bind_value(
                        config.ai_model, "max_tokens"
                    ).classes("w-32")
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.ai_model.context_window, min=512, max=131072, step=512).bind_value(
                        config.ai_model, "context_window"
                    ).classes("w-32")
                ...
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.ai_model.use_cot_swarm).bind_value(config.ai_model, "use_cot_swarm").props(
                        f"id={UI_IDS.SET_SWARM_COT}"
                    )
            ...
            # ==================== EXTERNAL LLMs ====================
            with (
                ui.expansion("External LLMs (Multi-LLM Consensus)", icon="cloud").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                ...
                with ui.column().classes("w-full gap-2"):
                    ...
                    ui.label("Get $5 free credits at https://console.anthropic.com/").classes(
                        Styles.LABEL_XS + " text-green-600"
                    )
                ...
                with ui.column().classes("w-full gap-2"):
                    ...
                    ui.label("FREE forever at https://aistudio.google.com/app/apikey").classes(
                        Styles.LABEL_XS + " text-green-600"
                    )
                    with ui.row().classes("w-full gap-2"):
                        ui.input(
                ...
                with ui.column().classes("w-full gap-2"):
                    ...
                    with ui.row().classes("w-full gap-2"):
                        ...
                        ui.select(options=["grok-beta"], value=config.external_llm.xai_model, label="Model").bind_value(
                            config.external_llm, "xai_model"
                        ).classes("w-64")
                ...
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.external_llm.cost_tracking_enabled).bind_value(
                        config.external_llm, "cost_tracking_enabled"
                    )
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(
                        value=config.external_llm.budget_limit, min=0, max=1000, step=5, format="$%.2f"
            ...
            # ==================== VOICE ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_VOICE, icon="mic").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # TTS Enabled
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.voice.tts_enabled).bind_value(config.voice, "tts_enabled").props(
                        f"id={UI_IDS.SET_TTS_ENABLE}"
                    )
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.slider(min=0.5, max=2.0, step=0.1, value=config.voice.voice_speed).bind_value(
                        config.voice, "voice_speed"
                    ).classes("w-48")
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.voice.recording_duration, min=1, max=30, step=1).bind_value(
                        config.voice, "recording_duration"
                    ).classes("w-24").props('suffix="s"')

            # ==================== RAG ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_RAG, icon="library_books").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # RAG Enabled
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.rag.enabled).bind_value(config.rag, "enabled").props(
                        f"id={UI_IDS.SET_RAG_ENABLE}"
                    )
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.rag.chunk_size, min=100, max=2000, step=50).bind_value(
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.rag.similarity_threshold, min=0, max=1, step=0.05, format="%.2f").bind_value(
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.rag.max_results, min=1, max=20, step=1).bind_value(
                        config.rag, "max_results"
                    ).classes("w-24")
            ...
            # ==================== CHAT ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_CHAT, icon="chat").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # Row of switches
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                ...
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.chat.history_days, min=1, max=365, step=1).bind_value(
                        config.chat, "history_days"
                    ).classes("w-24").props('suffix=" days"')

            # ==================== SYSTEM ====================
            with (
                ui.expansion(locale.SETTINGS_CAT_SYSTEM, icon="settings").classes("w-full " + Styles.CARD_BASE),
                ui.column().classes("w-full gap-4 p-4"),
            ):
                # API Port
                with ui.row().classes("w-full items-center gap-4"), ui.column().classes("flex-1"):
                    ...
                    ui.number(value=config.system.api_port, min=1024, max=65535, step=1).bind_value(
                        config.system, "api_port"
                    ).classes("w-32")
                ...
                ui.input(value=config.system.models_directory).bind_value(config.system, "models_directory").classes(
                    "w-full max-w-md"
                )
                ...
                with ui.row().classes("w-full items-center justify-between"), ui.column().classes("gap-0"):
                    ...
                    ui.switch(value=config.system.check_updates_on_startup).bind_value(
                        config.system, "check_updates_on_startup"
                    )
                ...
                ui.select(options=["DEBUG", "INFO", "WARNING", "ERROR"], value=config.system.log_level).bind_value(
        ...
        with ui.row().classes("w-full items-center justify-between p-4 border-t border-gray-200 dark:border-slate-700"):
            ui.button(
                locale.SETTINGS_RESET, icon=Icons.REFRESH, on_click=lambda: _reset_settings(dialog, on_save)
            ).props(f"flat id={UI_IDS.BTN_SET_RESET}").classes(Styles.TEXT_ERROR)

            with ui.row().classes("gap-2"):
                ui.button(locale.BTN_CANCEL, on_click=dialog.close).props(f"flat id={UI_IDS.BTN_CLOSE_DIALOG}")
                ui.button(
                    locale.SETTINGS_SAVE, icon=Icons.SAVE, on_click=lambda: _save_settings(dialog, on_save)
                ).classes(Styles.BTN_PRIMARY).props(f"id={UI_IDS.BTN_SET_SAVE}")
...
def _reset_settings(dialog, callback: Optional[Callable]):
    ...
    with ui.dialog() as confirm_dialog, ui.card():
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
            ...
            ui.button(locale.BTN_YES, on_click=lambda: _do_reset(dialog, confirm_dialog, callback)).classes(
                Styles.BTN_PRIMARY
            )
