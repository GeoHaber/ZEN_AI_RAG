...
def _demo_page_continued(
    avatar_text, chat_container, content, features, icon, indicator, on_click, rag_enhanced, role, scroll_area, sources
):
...
Try clicking the dark mode button to see it in action!""",
    ...
    # System message
    msg3 = ModernChatMessage(role="system", content="UI components loaded successfully")
...
The theme provides organized classes for all UI elements!""",
    ...
    with ui.footer().classes(MT.Layout.FOOTER):
        ...
        async def handle_send(message):
            """Handle message send"""
            # Add user message
            user_msg = ModernChatMessage(role="user", content=message, avatar_text="U")
    ...
    with ui.dialog() as info_dialog:
        with ui.card().classes(MT.Cards.PADDED + " w-96"):
            ui.label("About This Demo").classes(MT.combine(MT.TEXT_XL, MT.FONT_BOLD, "mb-4"))
...
All components are **separate from the backend**, making them easy to integrate and test!""").classes("mb-4")
            ...
            ui.button("Close", on_click=info_dialog.close).classes(MT.Buttons.PRIMARY_FULL + " w-full")

    # Info button in header (add retroactively)
    with ui.header().classes(MT.Layout.HEADER):
        ui.button(icon="info", on_click=info_dialog.open).props("flat").classes(MT.Buttons.ICON)
...
def demo_page():
    ...
    with ui.header().classes(MT.Layout.HEADER):
        ...
        ui.label("Modern UI Demo").classes(MT.combine(MT.TEXT_XL, MT.FONT_SEMIBOLD, MT.TextColors.PRIMARY))
        ...
        ui.button(icon="dark_mode" if not dark_mode.value else "light_mode", on_click=toggle_dark).props(
            "flat"
        ).classes(MT.Buttons.ICON)

    # Main content area
    scroll_area = ui.scroll_area().classes("w-full").style("height: calc(100vh - 200px); min-height: 300px;")
    ...
    async def handle_chip_click(value):
        ...
        if value == "user":
            msg = ModernChatMessage(
                role="user", content="Hello! This is a user message with **markdown** support.", avatar_text="U"
            )
...
**Note:** I support *Markdown* formatting!""",
            ...
            # Show AI response
            msg = ModernChatMessage(
                role="assistant", content="Typing indicator disappeared! This is the response.", avatar_text="Z"
            )
            msg.render(chat_container)
    ...
    # User message
    msg1 = ModernChatMessage(role="user", content="What can you tell me about the new UI?", avatar_text="U")
    msg1.render(chat_container)

    # AI response
    _demo_page_continued(
        avatar_text,
        chat_container,
        content,
        features,
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    ui.run(title="Modern UI Demo - ZenAI", port=8090, reload=False, dark=False)
