...
class UIHandlers:
    ...
    def add_message(self, role: str, content: str, enable_tts: bool = True):
        ...
        """
        with self.ui_state.chat_log, ui.row().classes(Styles.CHAT_ROW_USER if role == "user" else Styles.CHAT_ROW_AI):
            # Zena-style colors
            if role == "user":
            ...
            with ui.column().classes(align):
                ...
                if role in ("assistant", "assistant_rag") and enable_tts:
                        ...
                        try:
                            ...
                            async with httpx.AsyncClient() as client:
                                response = await client.post(
                                    "http://localhost:8001/voice/speak", json={"text": text_to_read}
                                )
                    ...
                    with ui.row().classes("items-center gap-1 mt-2"):
                        read_btn = (
                            ui.button(icon="volume_up", on_click=lambda: on_read_click(content))
                            .props("flat dense small")
    ...
    async def _stream_response_continued(
        ...
        try:
                    ...
                    try:
                        if chunk_count == 0:
                            distraction = random.choice(
                                getattr(self.locale, "LOADING_DISTRACTIONS", self.locale.LOADING_THINKING)
                            )
            ...
            if relevant_chunks:
                with (
                    sources_ui,
                    ui.expansion(self.locale.RAG_VIEW_SOURCES, icon=Icons.SOURCE).classes(
                        "w-full " + Styles.CARD_INFO + " rounded-xl mt-2"
                    ),
                    ui.column().classes("gap-1 p-2"),
                ):
                    for i, c in enumerate(relevant_chunks, 1):
                        ...
                        ui.label(f'📝 "{text_preview}"').classes(
                            Styles.LABEL_XS + " italic ml-4 mb-2 border-l-2 border-gray-300 pl-2 block"
                        )
    ...
    async def stream_response(self, prompt: str):
        ...
        if decision["type"] == "direct":
            logger.info("⚡ Fast Dispatch: Direct Response")
        ...
        with msg_row, ui.column().classes("w-full max-w-3xl"):
            ...
            msg_ui = ui.markdown(loading_msg).classes(
                Styles.CHAT_BUBBLE_AI + " p-4 rounded-3xl shadow-sm " + Styles.LOADING_PULSE + " w-full"
            )
            ...
            if self.conversation_memory:
                try:
                    final_prompt = self.conversation_memory.build_contextual_prompt(
                        prompt, session_id=self.ui_state.session_id
                    )
                except Exception:
                    pass
                ...
                try:
                    ...
                    if relevant_chunks:
                        for idx, c in enumerate(relevant_chunks):
                            logger.info(
                                f"[RAG] Chunk {idx + 1}: {c.get('title', 'Unknown')} (Score: {c.get('fusion_score', 'N/A')})"
                            )
    ...
    async def handle_council_mode(self, prompt: str):
        ...
        with msg_row:
            with ui.column().classes("w-full max-w-3xl"):
                # Thinking Bubble
                msg_ui = ui.markdown("**The Council is debating...**").classes(
                    Styles.CHAT_BUBBLE_AI + " p-4 rounded-3xl shadow-sm " + Styles.LOADING_PULSE
                )
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        with (
                            debate_ui,
                            ui.expansion(
                                f"Council Debate ({method.title()}, {confidence:.0%} Agreement)", icon="groups"
                            ).classes(
                                "w-full bg-blue-50 dark:bg-slate-800 rounded-xl border border-blue-100 dark:border-slate-700"
                            ),
                            ui.column().classes("p-2 gap-2"),
                        ):
                            for i, exp in enumerate(experts):
                                model_name = exp.get("model", f"Expert {i + 1}")
                                exp_conf = exp.get("confidence", 0.0)
                                with (
                                    ui.card().classes("w-full p-2 bg-white dark:bg-slate-900"),
                                    ...
                                    ui.badge(f"{exp_conf:.0%} conf", color="grey").props("outline dense").classes(
                                        "text-[10px]"
                                    )

                                    ui.markdown(exp.get("content", "")).classes(
    ...
    async def run_internal_check(self, monkey=False):
        ...
        msg_ui = ui.markdown("⏳ *Running diagnostics...*").classes(
            Styles.CHAT_BUBBLE_AI + " p-4 rounded-3xl shadow-sm w-full font-mono text-xs"
        )
