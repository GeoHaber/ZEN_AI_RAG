...
class UIHandlers:
    ...
    async def _stream_response_continued(self, final_prompt, full_text, msg_ui, relevant_chunks, sources_ui):
        ...
        try:
            ...
            # Sources disclosure
            if relevant_chunks:
                with sources_ui, ui.expansion(self.locale.RAG_VIEW_SOURCES, icon=Icons.SOURCE).classes('w-full ' + Styles.CARD_INFO + ' rounded-xl mt-2'), ui.column().classes('gap-1 p-2'):
             ...
             with ui.column().classes('w-full max-w-3xl'):
                 ...
                 try:
                     ...
                     if resp.status_code == 200:
                         ...
                         # Show Debate
                         with debate_ui, ui.expansion(f"Council Debate ({method.title()}, {confidence:.0%} Agreement)", icon='groups').classes('w-full bg-blue-50 dark:bg-slate-800 rounded-xl border border-blue-100 dark:border-slate-700'), ui.column().classes('p-2 gap-2'):
                             for i, exp in enumerate(experts):
                                 model_name = exp.get('model', f'Expert {i+1}')
                                 exp_conf = exp.get('confidence', 0.0)
                                 with ui.card().classes('w-full p-2 bg-white dark:bg-slate-900'), ui.row().classes('items-center justify-between w-full'):
