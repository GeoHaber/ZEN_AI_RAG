...
def build_header(ui_state, drawer, locale, open_gallery=None):
    ...
    with ui.header().classes(Styles.HEADER) as header:
        ui.button(on_click=lambda: drawer.toggle(), icon=Icons.MENU).props("flat round id=ui-drawer-btn").classes(
            Styles.TEXT_PRIMARY
        )
        ...
        with ui.row().classes("items-center gap-2"):
            with ui.row().classes(
                "items-center gap-1.5 px-3 py-1 rounded-full bg-gray-50/50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700"
            ):
                ui_state.status_dot = (
                    ui.label("●").classes("text-green-500 animate-pulse text-[14px]").props("id=header-status-dot")
                )
                ui_state.status_indicator = (
                    ui.label("ONLINE")
                    .classes("text-[10px] font-black tracking-widest text-green-500")
                    .props("id=header-status-label")
                )

            # Voice status indicator (hidden by default, shown when recording)
            ui_state.voice_status = (
                ui.label("🎙️ REC")
                .classes("text-[10px] font-black tracking-widest text-red-500 animate-pulse hidden")
                .props("id=header-voice-label")
            )

            # Model Gallery Button (The "Cards")
            if open_gallery:
                with (
                    ui.button(icon="smart_toy", on_click=open_gallery)
                    .props("flat round dense")
                    .classes(Styles.TEXT_MUTED + " hover:text-purple-500 transition-colors")
                ):
                    ui.tooltip("The Council (Models)")

            ui.button(icon=Icons.PERSON).props("flat round dense").classes(Styles.TEXT_MUTED)
        ui_state.status_text = ui.label(locale.CHAT_READY).classes(
            Styles.TEXT_SECONDARY + " text-xs md:text-sm ml-auto hidden xs:block"
        )
    return header
...
def build_footer(ui_state, handlers, locale):
    ...
    with (
        ui.footer().classes("bg-transparent border-0 p-3 md:p-4") as footer,
        ui.column().classes("w-full max-w-3xl mx-auto gap-2"),
    ):
        ...
        with (
            ...
            with ui.row().classes("items-center gap-2 w-full"):
                ...
                ui.button(on_click=lambda: uploader.run_method("pickFiles"), icon=Icons.ATTACH).props(
                    f"flat round dense id={UI_IDS.BTN_ATTACH}"
                ).classes("text-gray-400 hover:text-blue-500 transition-colors")

                # Input field - grows to fill space
                placeholder_text = (
                    getattr(locale, "CHAT_PLACEHOLDER_ZENA", "Ask anything...")
                    if config.zena_mode_enabled
                    else getattr(locale, "CHAT_PLACEHOLDER", "Type a message...")
                )
                ui_state.user_input = (
                    ui.input(placeholder=placeholder_text)
                    .props(f"borderless dense autogrow id={UI_IDS.INPUT_CHAT}")
                ...
                # Voice button
                ui.button(icon=Icons.RECORD, on_click=handlers.on_voice_click).props(
                    f"flat round dense id={UI_IDS.BTN_VOICE}"
                ).classes("text-gray-400 hover:text-purple-500 transition-colors").tooltip(
                    "Click to toggle voice recording"
                )

                # Send button - prominent
                ui.button(icon=Icons.SEND, on_click=lambda: handlers.handle_send(ui_state.user_input.value)).props(
                    f"round dense unelevated id={UI_IDS.BTN_SEND}"
                ).classes(
...
def build_chat_area(ui_state):
    ...
    ui_state.scroll_container = (
        ui.scroll_area().classes("w-full").style("height: calc(100vh - 160px); min-height: 250px;")
    )
    with ui_state.scroll_container:
        ui_state.chat_log = (
            ui.column()
            .classes(
                "w-full max-w-4xl mx-auto p-2 md:p-4 space-y-3 md:space-y-4 "
                + Styles.CHAT_CONTAINER_LIGHT
                + " "
                + Styles.CHAT_CONTAINER_DARK
            )
            .props("id=chat-log-container")
        )
...
def build_page(ui_state, handlers, drawer_factory, rag_dialog_factory):
    ...
    with ui_state.chat_log, ui.row().classes("w-full justify-center gap-2 mt-4 flex-wrap"):
        for item in EXAMPLES:
            ...
            ui.button(item["title"], on_click=lambda p=item["prompt"]: make_handler(p)).props(
                "outline rounded-full dense"
            ).classes("text-[11px] px-3 py-1 normal-case hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors")
