...
class DialogsFactory:
    ...
    def setup_common_dialogs(backend, app_state, on_settings_save=None, on_language_change=None, on_dark_mode=None):
        ...
        with ui.dialog() as model_dialog, ui.card().classes("w-full max-w-lg p-6 bg-white dark:bg-slate-800"):
            ...
            repo_input = (
                ui.input("HuggingFace Repo", placeholder="TheBloke/Llama-2-7B-Chat-GGUF")
                .classes("w-full mb-2")
                .props("outlined dense")
            )
            file_input = (
                ui.input("Filename", placeholder="llama-2-7b-chat.Q4_K_M.gguf")
                .classes("w-full mb-4")
                .props("outlined dense")
            )
        ...
        settings_dialog = create_settings_dialog(
            on_save=on_settings_save, on_language_change=on_language_change, on_dark_mode_change=on_dark_mode
        )
