...
class _UIActionsBase:
    ...
    async def start_batch(self, files_input, progress_container, progress_label, progress_bar, batch_btn):
        ...
        if len(paths) == 1 and os.path.isdir(paths[0]):
            ...
            paths = [
                str(f) for f in dir_path.glob("**/*") if f.is_file() and f.suffix in (".py", ".txt", ".md", ".ipynb")
            ]
            if not paths:
        ...
        try:
            ...
            with ui.notification(
                f"Batch Complete: {result['completed']} files analyzed!", color="positive", timeout=10000
            ):
                if paths:
                    folder = str(Path(paths[0]).parent)
                    ui.button("Open Folder", icon="folder", on_click=lambda f=folder: os.startfile(f)).props(
                        "flat color=white"
                    )
        except Exception as e:
...
class UIActions(_UIActionsBase):
    ...
    def get_model_info(self, filename, MODEL_INFO):
        ...
        if "qwen" in fname_lower and "coder" in fname_lower:
            return {
                "name": "Qwen Coder",
                "desc": "Coding specialist model",
                "size": "~5GB",
                "icon": "💻",
                "good_for": ["Coding", "Debugging"],
                "speed": "Medium",
                "quality": "Excellent",
            }
            ...
            return {
                "name": "Qwen",
                "desc": "Alibaba's AI model",
                "size": "~4GB",
                "icon": "🤖",
                "good_for": ["Chat", "Writing"],
                "speed": "Medium",
                "quality": "Good",
            }
            ...
            return {
                "name": "CodeLlama",
                "desc": "Meta's code model",
                "size": "~4GB",
                "icon": "🦙💻",
                "good_for": ["Coding", "Code Review"],
                "speed": "Medium",
                "quality": "Good",
            }
            ...
            return {
                "name": "Yi",
                "desc": "01.AI's bilingual model",
                "size": "~4GB",
                "icon": "🎯",
                "good_for": ["Multilingual", "Chat"],
                "speed": "Medium",
                "quality": "Good",
            }
        else:
    ...
    async def switch_to_model(self, model_file, info, ui_elements):
        ...
        try:
            response = await asyncio.to_thread(
                requests.post, "http://127.0.0.1:8002/models/load", json={"model": model_file}, timeout=30
            )
    ...
    async def download_model(self, model):
        ...
        try:
            ...
            response = await asyncio.to_thread(
                ...
                json={"repo_id": model["repo"], "filename": model["file"]},
                timeout=10,
            )
            if response.status_code == 200:
    ...
    def open_voice_lab(self):
        ...
        with ui.dialog().props("maximized") as lab_dialog:
            with (
                ui.card().classes("w-full h-full p-0 no-shadow"),
                ui.row().classes("w-full p-2 bg-gray-100 dark:bg-slate-800 items-center justify-between border-b"),
            ):
                ...
                ui.html(
                    '<iframe src="http://localhost:8002/voice/lab" style="width:100%; height:calc(100% - 50px); border:none;"></iframe>',
                    sanitize=False,
    ...
    async def open_judge(self):
        ...
        try:
            llm_url = (
                self.config.get("LLM_API_URL", "http://127.0.0.1:8001")
                if isinstance(self.config, dict)
                else getattr(self.config, "LLM_API_URL", "http://127.0.0.1:8001")
            )
        ...
        with (
            ui.dialog().props("maximized") as judge_dialog,
            ui.card().classes("w-full h-full p-0 overflow-hidden"),
            ui.row().classes("w-full p-4 items-center justify-between border-b"),
        ):
        ...
        try:
            ...
            try:
                result = await asyncio.to_thread(
                    subprocess.run, ["_bin/llama-server.exe", "--version"], capture_output=True, text=True, timeout=5
                )
