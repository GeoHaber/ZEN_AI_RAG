    ...
    def render(self, container=None):
        ...
        with msg_row:
            ...
            if self.role != 'user' and self.role != 'system':
                ui.avatar(
                    self.avatar_text,
                    color='purple' if not self.rag_enhanced else 'blue',
                    text_color='white'
        ...
        with row:
            ...
            with ui.column().classes('gap-1'):
                ...
                with ui.card().classes(
                    MT.ChatBubbles.AI_FULL + ' flex items-center justify-center min-h-12'
                ):
                    ui.html('''
...
class ModernInputBar:
    ...
    def render(self, container=None):
        ...
        with input_col:
            ...
            self.attachment_preview = ui.label('').classes(
                MT.Cards.INFO + ' text-sm self-start'
            ).props('visible=false')
            ...
            with ui.row().classes(MT.Inputs.CHAT_BAR):
                ...
                ).classes(MT.Inputs.CHAT).props('borderless')
    ...
    def render(self, container=None):
        ...
        with chips_col:
            ui.label('Quick Actions').classes(
                MT.combine(MT.TEXT_SM, MT.TextColors.MUTED, 'ml-1')
            )
            ...
            with ui.row().classes('gap-2 flex-wrap'):
                for action in self.actions:
                    ...
                    ui.button(label, on_click=chip_click).classes(
                        MT.Badges.CHIP
...
class ModernWelcomeMessage:
    ...
    def render(self, container=None):
        ...
        welcome_col = ui.column().classes(
            'w-full max-w-2xl mx-auto text-center gap-4 p-8 animate-fade-in'
        )
        if container:
...
def create_modern_chat_container(scroll_container):
    ...
    with scroll_container:
        chat_log = ui.column().classes(
            MT.Layout.CHAT_CONTAINER
        )
    return chat_log
