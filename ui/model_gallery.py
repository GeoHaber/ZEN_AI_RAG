...
def create_model_gallery(ui_state, app_state):
    ...
    with (
        ui.dialog() as dialog,
        ui.card().classes(
            "w-full max-w-4xl h-[80vh] bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700"
        ),
    ):
        # Header
        with ui.row().classes(
            "w-full items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700"
        ):
                    ...
                    with grid_container:
                        for model_file in models:
                            ...
                            with ui.card().classes(
                                "flex flex-col gap-2 p-3 hover:shadow-lg transition-shadow bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 h-[180px] justify-between relative overflow-hidden group"
                            ):
                                # Icon & Name
                                with ui.row().classes("items-start gap-3 w-full"):
                                    # Icon/Image Container - Scaled to fit
                                    with ui.column().classes(
                                        "w-12 h-12 flex-shrink-0 items-center justify-center bg-slate-100 dark:bg-slate-700 rounded-lg overflow-hidden"
                                    ):
                                        ui.label(meta["icon"]).classes(
                                            "text-3xl select-none transform transition-transform group-hover:scale-110"
                                        )

                                    with ui.column().classes("gap-0 flex-grow min-w-0"):
                                        ui.label(meta["name"]).classes(
                                            "font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight truncate w-full"
                                        ).props(f'title="{meta["name"]}"')
                                        ui.label(meta["size"]).classes("text-[10px] text-slate-400")

                                ui.label(meta["desc"]).classes(
                                    "text-xs text-slate-600 dark:text-slate-400 line-clamp-2 leading-relaxed"
                                )
                                ...
                                with ui.row().classes("gap-1 flex-wrap"):
                                    for tag in meta.get("good_for", [])[:2]:
                                        ui.label(tag).classes(
                                            "text-[9px] px-1.5 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 rounded-md border border-blue-100 dark:border-blue-800/50"
                                        )
                                ...
                                with ui.row().classes("w-full justify-end gap-2"):
                                    ...
                                    def swap(m=model_file):
                                        ...
                                        requests.post(
                                            f"http://127.0.0.1:{config.mgmt_port}/swap", json={"model": m}, timeout=30
                                        )
                                        dialog.close()
                                    ...
                                    def launch(m=model_file):
                                        ...
                                        requests.post(
                                            f"http://127.0.0.1:{config.mgmt_port}/swarm/launch",
                                            json={"model": m, "port": 8005},
                                            timeout=30,
                                        )  # Hardcoded for demo
