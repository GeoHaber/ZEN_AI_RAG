...
class TestZombieCleanup:
    ...
    def test_kill_process_by_port_works(self):
        ...
        script = (
            f"import socket, time; s=socket.socket(); s.bind(('127.0.0.1', {test_port})); s.listen(1); time.sleep(10)"
        )
        proc = subprocess.Popen([sys.executable, "-c", script], shell=False)
    ...
    def test_start_server_cleanup_logic(self):
        ...
        with (
            patch("start_llm.kill_process_by_port") as mock_kill_port,
