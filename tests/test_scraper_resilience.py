...
def test_scraper_exponential_backoff():
    ...
    mock_responses = [
        ...
        MagicMock(
            status_code=200,
            text="<html><title>Success</title><body>This is a substantial piece of content that exceeds the 100 character threshold to ensure the scraper actually saves it as a valid document for RAG indexing. It needs to be long enough to pass the filters.</body></html>",
        ),
    ]
...
def test_scraper_antibot_detection():
    ...
    mock_response = MagicMock(
        status_code=200, text="<html><body>Please complete the security check by cloudflare</body></html>"
    )
