...
def _do_main_setup():
    ...
    rag_instance, rag_init_time = measure("RAG Init (cold)", lambda: LocalRAG(cache_dir=temp_dir))
    ...
    _, index_time = measure("Index 10 Documents", lambda: rag_instance.build_index(test_docs))
...
def main():
    ...
    print("\n--- RAG Search ---")
    _, search_cold = measure("Search (cold)", lambda: rag_instance.search("artificial intelligence", k=3))
    results["search_cold"] = search_cold

    _, search_warm = measure("Search (warm)", lambda: rag_instance.search("test document", k=3))
