...
MODELS = ["SmolLM2-135M", "Qwen-0.5B", "Llama-3.2-3B", "Phi-3.5-mini", "Mistral-7B"]
...
def send_message(text: str):
    ...
    if is_rag_reply:
        ...
        reply = random.choice(["Mock response OK", "Test reply ✓", "Bot says hello! 🤖"])
...
ALL_ACTIONS = [
    ...
    (
        "Send: Long",
        lambda: send_message("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor."),
    ),
            ...
            if msg["role"] == "user":
                with ui.row().classes("w-full justify-end"):
                    ui.label(msg["text"]).classes(
                        "bg-blue-100 dark:bg-blue-900 text-blue-900 dark:text-blue-100 p-2 rounded-lg max-w-[80%]"
                    )
            else:
                with ui.row().classes("w-full justify-start items-start gap-2"):
                    ...
                    ui.label(msg["text"]).classes(
                        f"{bg_class} text-gray-800 dark:text-gray-100 p-2 rounded-lg max-w-[80%] whitespace-pre-wrap"
                    )
...
def stress_status():
    ...
    if state.stress_running:
        ui.label(f"🔥 STRESS TEST: Phase {state.stress_phase}/5 | Actions: {state.action_count}").classes(
...
def drawer_content():
    ...
    for item in ["Home", "Settings", "Scan Folder", "Download Model", "About"]:
        ui.button(item, icon="chevron_right", on_click=lambda i=item: menu_click(i)).props("flat align=left").classes(
            f"w-full justify-start mb-1 {btn_text_class}"
        )
...
**Purpose:** Find UI bugs, race conditions, and crashes.
        ...
        with ui.row().classes("gap-2 w-full"):
            ui.button("🚀 START", on_click=lambda: [dialog.close(), asyncio.create_task(run_stress_test())]).props(
                "color=positive"
            ).classes("flex-grow")
...
def _do_build_ui_setup():
    ...
    header_panel.classes(
        "bg-white dark:bg-slate-800 text-gray-800 dark:text-white border-b border-gray-200 dark:border-slate-700"
    )

    with header_panel, ui.row().classes("w-full items-center justify-between"):
        # Left: Menu toggle (actually toggles drawer)
        ui.button(icon="menu", on_click=lambda: drawer_panel.toggle()).props("flat").classes(
            "text-gray-700 dark:text-white"
        )

        # Dark mode toggle (single button)
        ui.button(icon="dark_mode", on_click=toggle_dark_mode).props("flat").classes(
            "text-gray-700 dark:text-white"
        ...
        with ui.row().classes("gap-2 items-center"):
            ui.button(icon="volume_up", on_click=toggle_tts).props("flat").classes(
                "text-gray-700 dark:text-white"
            ).tooltip("TTS")

            # Scan Button (hidden by default, shown when RAG enabled)
            scan_button = (
                ui.button("Start Scanning", icon="book", on_click=scan_action)
                .props("flat")
            ...
            ui.switch("Scan & Learn", value=False, on_change=on_rag_toggle).props("color=cyan keep-color").classes(
                "text-gray-700 dark:text-white"
            )
...
def build_ui():
    ...
    with ui.row().classes("w-full flex-grow p-4 gap-4 main-content-row"):
        ...
        with ui.column().classes("w-full md:w-1/2 gap-2 content-col"):  # Responsive: full width on mobile
            ...
            with ui.row().classes("gap-2 mt-2"):
                ui.button("Run Random", icon="shuffle", on_click=lambda: asyncio.create_task(run_random_quick())).props(
                    "color=primary"
                )
    ...
    footer_panel.classes(
        "bg-white dark:bg-slate-800 text-gray-800 dark:text-white border-t border-gray-200 dark:border-slate-700"
    )
    with footer_panel, ui.row().classes("w-full items-center gap-2"):
        ...
        msg_input = (
            ...
            .classes("flex-grow bg-white dark:bg-slate-900 rounded")
