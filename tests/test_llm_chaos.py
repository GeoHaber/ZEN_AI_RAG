    ...
    async def test_concurrency_flood(self):
        ...
        async def send_request(session, i):
            ...
            try:
                ...
                async with session.post(f"http://127.0.0.1:{config.llm_port}/completion", json=payload, timeout=5) as resp:
                    return resp.status
    ...
    async def test_tokenizer_stress(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(f"http://127.0.0.1:{config.llm_port}/completion", json=payload, timeout=10) as resp:
