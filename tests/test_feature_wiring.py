...
class _TestFeatureWiringBase(unittest.TestCase):
    ...
    def test_tutorial_button_exists_in_ui(self):
        ...
        self.assertIn(
            ...
            "Tutorial function should be wired to a button in ui_components.py"
        )
    ...
    def test_tutorial_button_has_id(self):
        ...
        self.assertIn(
            'ui-tour-btn',
            self.ui_components_code,
            "Tutorial button should have id='ui-tour-btn' for test automation"
        )
    ...
    def test_help_docs_indexed_on_startup(self):
        ...
        self.assertIn(
            'index_internal_docs',
            self.zena_code,
            "index_internal_docs should be called in zena.py"
        )
    ...
    def test_voice_lab_endpoint_exists(self):
        ...
        self.assertIn(
            '/voice/lab',
            self.asgi_server_code,
            "Voice Lab endpoint '/voice/lab' should exist in asgi_server.py"
        )
...
class TestFeatureWiring(_TestFeatureWiringBase, unittest.TestCase):
    ...
    def test_model_switch_button_exists(self):
        ...
        self.assertIn(
            'switch_to_model',
            self.ui_components_code,
            "switch_to_model function should exist for model switching"
        )
    ...
    def test_settings_dialog_wired(self):
        ...
        self.assertIn(
            ...
            self.ui_components_code,
            "Settings dialog should be openable from UI"
        )
    ...
    def test_quality_dashboard_exists(self):
        ...
        self.assertIn(
            'create_quality_tab',
            self.ui_components_code,
            "create_quality_tab should be called in the Judge dialog"
        )
    ...
    def test_new_chat_button_exists(self):
        ...
        self.assertIn(
            'BTN_NEW_CHAT',
            self.ui_components_code,
            "New Chat button with ID should exist in UI"
        )
    ...
    def test_asgi_has_model_list_endpoint(self):
        ...
        self.assertIn(
            '/list',
            self.asgi_server_code,
            "Model list endpoint should exist"
        )
        ...
        for id_name in critical_ids:
            with self.subTest(id_name=id_name):
                self.assertIn(
                    id_name,
                    self.registry_code,
                    f"UI_IDS.{id_name} should be defined in registry.py"
                )
...
class TestImportIntegrity(unittest.TestCase):
    ...
    def test_imported_tutorial_is_used(self):
        ...
        self.assertIn(
            'start_tutorial',
            ui_code,
            "start_tutorial should be used in ui_components.py"
        )
    ...
    def test_imported_help_system_is_used(self):
        ...
        if import_match:
            ...
            self.assertRegex(
                after_import,
                r'index_internal_docs\s*\(',
                "index_internal_docs is imported but never called in zena.py"
            )
