...
class SoakTester:
    ...
    def run_instance(self, worker_id, model, prompt):
        ...
        cmd = [
            ...
            prompt,
            "-n",
            str(random.randint(64, 256)),  # Variable output length
            "-c",
            "2048",
            "--temp",
            str(temp),
    ...
    def run(self):
        ...
        while (time.time() - self.start_time) < self.target_duration:
            ...
            self.log(
                f"Stats: TotalReq={self.total_requests} FailRate={self.failed_requests / self.total_requests * 100:.1f}% Elapsed={elapsed_total / 3600:.2f}h"
            )
