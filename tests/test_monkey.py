...
class MonkeyStateTracker:
    ...
    def log_action(self, action_type, target, result="OK"):
        """Log action."""
        self.actions.append(
            {"time": time.time() - self.start_time, "type": action_type, "target": target, "result": result}
        )

    def log_error(self, action, error):
        """Log error."""
        self.errors.append({"time": time.time() - self.start_time, "action": action, "error": str(error)})
...
class MockUIHandler:
    ...
    def __init__(self, llm_online=True):
        ...
        self.chat_history = []
        self.settings = {"dark_mode": False, "rag_enabled": True, "language": "en"}
    ...
    def _send_message(self):
        ...
        if self.llm_online:
            # Simulate response
            self.chat_history.append({"role": "assistant", "content": f"Response to: {msg[:50]}"})
