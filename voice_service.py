...
logger = logging.getLogger("ZenAIVoice")
...
class VoiceService:
    ...
    def load_stt_model(self):
        ...
        self.stt_model = WhisperModel(
            self.stt_model_name, device="cpu", compute_type="int8", download_root=str(self.model_dir / "whisper")
        )
    ...
    def ensure_piper_model(self):
        ...
        if self.tts_voice_name != "en_US-lessac-medium":
            logger.warning(
                f"[TTS] Auto-download only supports 'en_US-lessac-medium'. Please check path manually for {self.tts_voice_name}"
            )
            return onnx_file, json_file
        ...
        with self._lock:
            ...
            try:
                ...
                segments, info = self.stt_model.transcribe(
                    transcribe_target, language=language if language != "auto" else None, beam_size=5, vad_filter=True
                )
...
class StreamProcessor:
    ...
    def process(self) -> dict:
        ...
        try:
            ...
            if full_text != self.last_text:
                ...
                return {"text": full_text, "is_final": False}
    ...
    def finish(self) -> dict:
        ...
        try:
            ...
            segments, _ = self.model.transcribe(audio_float32, language="en", beam_size=5, word_timestamps=False)
            ...
            return {"text": final_text, "is_final": True}
...
def get_voice_service(
    model_dir: Path, stt_model: str = "base.en", tts_voice: str = "en_US-lessac-medium"
) -> VoiceService:
...
if __name__ == "__main__":
    ...
    safe_print("[Test] Loading STT model...")
    service.load_stt_model()
    safe_print("[Test] STT model ready!")

    safe_print("[Test] Loading TTS model...")
    service.load_tts_model()
    safe_print("[Test] TTS model ready!")

    # Test TTS
    safe_print("[Test] Generating speech...")
    test_text = "Hello! This is ZenAI's local voice system. Everything runs on your computer, with complete privacy."
    audio_data = service.synthesize_speech(test_text)
    safe_print(f"[Test] Generated {len(audio_data)} bytes of audio")

    # Save test audio
    test_file = model_dir / "test_output.wav"
    with open(test_file, "wb") as f:
        f.write(audio_data)
    safe_print(f"[Test] Saved to: {test_file}")

    safe_print("[Test] Voice service is working!")
